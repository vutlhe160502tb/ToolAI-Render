...
export async function POST(request: NextRequest) {
  try {
    ...
    const person_image = formData.get('person_image');
    const quality = formData.get('quality') as string | null;
    const user_id = formData.get('user_id') as string;

    if (!product_image || !person_image) return NextResponse.json({ message: 'Missing files' }, { status: 400 });
    ...
    backendFormData.append('person_image', person_image);
