...
import { Search, Heart, Download, CheckCircle, Clock, Settings, XCircle, Image as ImageIcon, ArrowLeft } from 'lucide-react';
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      if (response.data && response.data.jobs) {
        // Filter chỉ lấy các job liên quan đến ảnh
        const imageJobs = response.data.jobs.filter((job: VideoJob) => 
          IMAGE_FEATURE_TYPES.includes(job.feature_type)
        );
        setJobs(imageJobs);
      } else {
        setJobs([]);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            <h1 className="text-white text-2xl md:text-3xl font-bold mb-2 flex items-center gap-2">
              <ImageIcon className="w-8 h-8 text-[#D344FF]" />
              Ảnh
            </h1>
            <p className="text-gray-400 text-sm">Tất cả các ảnh đã tạo</p>
          </div>
