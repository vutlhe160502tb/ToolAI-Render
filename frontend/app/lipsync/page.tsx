...
import { Search, Heart, Download, CheckCircle, Clock, Settings, XCircle, Mic, ArrowLeft } from 'lucide-react';
...
export default function LipsyncPage() {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      if (response.data && response.data.jobs) {
        // Filter chỉ lấy các job liên quan đến lipsync
        const lipsyncJobs = response.data.jobs.filter((job: VideoJob) => 
          LIPSYNC_FEATURE_TYPES.includes(job.feature_type)
        );
        setJobs(lipsyncJobs);
      } else {
        setJobs([]);
  ...
  const getFeatureName = (featureType: string) => {
    const names: Record<string, string> = {
      'product-intro-audio': 'Giới Thiệu Sản Phẩm',
      'lip-sync': 'Lips Sync',
    };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            <h1 className="text-white text-2xl md:text-3xl font-bold mb-2 flex items-center gap-2">
              <Mic className="w-8 h-8 text-[#D344FF]" />
              Lipsync
            </h1>
            <p className="text-gray-400 text-sm">Tất cả các video lipsync và product-intro-audio</p>
          </div>
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-full py-20">
                <div className="text-center">
                  ...
                  <p className="text-gray-400">Chưa có video lipsync nào</p>
                </div>
              </div>
            ) : (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="aspect-square bg-[#0a0519] relative overflow-hidden">
                              ...
                              {/* Feature type badge */}
                              <div className="absolute top-2 left-10 bg-black/70 backdrop-blur-sm px-2 py-1 rounded text-xs text-white">
